<template>
  <div v-for="(comment, index) in comments" :index="index" class="g-display-flex g-padding-2vh-0">
    <div class="g-min-width-40px g-width-10vw g-text-align-right">
      <el-avatar :src="comment.icon" @error="handleUserLoadError(comment)">
    ...
    <div class="g-flex-auto g-padding-0-1vw">
      <el-config-provider size="small">
        <div class="g-height-3vh g-min-height-30px">
          <el-link :href="comment.home" target="_blank">{{ comment.name }}</el-link>
...
const { article, quote } = withDefaults(
  defineProps<{
...
app.user
  ...
  .then(result => comments.value.push(...result))

function handleUserLoadError (comment: CommentType) {
  console.warn(`User: ${comment.name} load Failed! Url: ${comment.icon}`)
}
